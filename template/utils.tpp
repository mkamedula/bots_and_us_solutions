...
void save1DPng(const Container& data, const std::string& file_name, size_t ignored_bits)
{
    ...
    if (ignored_bits >= data.size() * 8)
    {
        throw exceptions::SaveFileException("Could not save the file. Requested saving an empty PNG image.");
    }

    size_t width = data.size() * 8 - ignored_bits;
